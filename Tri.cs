...
/// <remarks>
///     Represents a triangular tile in a two-dimensional grid in which the tiles alternative between being up-pointing and
///     down-pointing triangles. Each tri is represented as a pair of coordinates (X, Y), where X counts the tris in a row and
///     Y identifies the row. The (0, 0) tri is an up-pointing one.</remarks>
/// <image type="raw">
...
public struct Tri(int x, int y) : IEquatable<Tri>, INeighbor<Tri>, INeighbor<object>, IHasSvgGeometry
{
    ...
    /// <remarks>
    ///     With <paramref name="dx"/> and <paramref name="dy"/> equal to <c>0</c>, the bottom-left tri has an x-coordinate of
    ///     <c>0</c> for odd values of <paramref name="sideLength"/> and <c>1</c> for even values, while the top tri has a
    ///     y-coordinate of <c>0</c>.</remarks>
    ...
    ///     Offset to shift the y-coordinates by. Default is <c>0</c>.</param>
    /// <remarks>
    ///     With <paramref name="dx"/> and <paramref name="dy"/> equal to <c>0</c>, the top-left tri has coordinates <c>(1,
    ///     0)</c>.</remarks>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     <paramref name="sideLength"/> was zero or negative.</exception>
    /// <exception cref="ArgumentException">
    ...
    ///     Offset to shift the y-coordinates by. Default is <c>0</c>.</param>
    /// <remarks>
    ///     With <paramref name="dx"/> and <paramref name="dy"/> equal to <c>0</c>, the tris at the leftmost corner have an
    ///     x-coordinate of <c>0</c> for odd values of <paramref name="sideLength"/> and <c>1</c> for even values, while the
    ///     top row of tris has y-coordinates of <c>0</c>.</remarks>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     <paramref name="sideLength"/> was zero or negative.</exception>
    /// <exception cref="ArgumentException">
    ///     The values of <paramref name="dx"/> and <paramref name="dy"/> are not both even or both odd.</exception>
    ...
    /// <summary>
    ///     Returns the vertices along the perimeter of this <see cref="Tri"/>, going clockwise from the top (up-pointing) or
    ///     top-left (down-pointing).</summary>
    ...
    private const double _cos60 = .5;
    private const double _sin60 = .86602540378443864676372317075294;

    /// <inheritdoc/>
    public readonly PointD Center => new(X * (1.5d * _cos60), (IsUpPointing ? Y + 2d / 3d : Y + 1d / 3d) * (1.5d * _sin60));

    /// <inheritdoc/>
    public override readonly string ToString() => $"T({X},{Y})";

    /// <summary>
    ///     Describes a 2D grid of triangular cells.</summary>
    /// <remarks>
    ...
    public class Vertex : Coordinates.Vertex
    {
        ...
        /// <inheritdoc/>
        public override PointD Point => new(Tri.X * _cos60 * 1.5, Tri.Y * _sin60 * 1.5);
    }

    /// <summary>Equality operator.</summary>
    public static bool operator ==(Tri left, Tri right) => left.Equals(right);
