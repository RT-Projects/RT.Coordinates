...
{
    /// <summary>
    ///     The A component of this vector, which extends outward from the origin at a 54° angle counter-clockwise (SVG) or
    ///     clockwise (geometry) from the x-axis.</summary>
    public int A { get; private set; } = a;
    /// <summary>
    ///     The B component of this vector, which extends outward from the origin at an 18° angle counter-clockwise (SVG) or
    ///     clockwise (geometry) from the x-axis.</summary>
    ...
    /// <summary>
    ///     Returns a <see cref="Pentavector"/> describing a unit vector at a 54° angle counter-clockwise (SVG) or clockwise
    ///     (geometry) from the x-axis.</summary>
    public static readonly Pentavector BaseA = new(1, 0, 0, 0);
    /// <summary>
    ///     Returns a <see cref="Pentavector"/> describing a unit vector at a 18° angle counter-clockwise (SVG) or clockwise
    ///     (geometry) from the x-axis.</summary>
    public static readonly Pentavector BaseB = new(0, 1, 0, 0);
    /// <summary>
    ///     Returns a <see cref="Pentavector"/> describing a unit vector at a 18° angle clockwise (SVG) or counter-clockwise
    ///     (geometry) from the x-axis.</summary>
    public static readonly Pentavector BaseC = new(0, 0, 1, 0);
    /// <summary>
    ///     Returns a <see cref="Pentavector"/> describing a unit vector at a 54° angle clockwise (SVG) or counter-clockwise
    ///     (geometry) from the x-axis.</summary>
    public static readonly Pentavector BaseD = new(0, 0, 0, 1);
    /// <summary>Returns a <see cref="Pentavector"/> describing a unit vector pointing straight down (SVG) or up (geometry).</summary>
    ...
    /// <param name="angle">
    ///     The angle, specified as a multiple of 36° going clockwise (SVG) or counter-clockwise (geometry) from vertically
    ///     straight up (SVG) or down (geometry).</param>
    /// <remarks>
    ...
    /// <summary>
    ///     Returns a new <see cref="Pentavector"/> whose direction is the same as this vector, but with a length multiplied
    ///     by 1/φ. φ is the golden ratio ((√(5)+1)/2).</summary>
    public readonly Pentavector DivideByPhi => new(B - D, C + D - B, A + B - C, C - A);
    /// <summary>
    ///     Returns a new <see cref="Pentavector"/> whose direction is the same as this vector, but with a length multiplied
    ///     by φ. φ is the golden ratio ((√(5)+1)/2).</summary>
    ...
    private const double _sin18 = .30901699437494742410;
    private const double _cos18 = .95105651629515357210;
    private const double _sin54 = .80901699437494742410;
    private const double _cos54 = .58778525229247312918;

    /// <summary>Returns the 2D point represented by this vector.</summary>
    public readonly PointD Point => new(_cos54 * (A + D) + _cos18 * (B + C), _sin54 * (D - A) + _sin18 * (C - B));
