...
/// <remarks>
///     <para>
///         Use this to describe vertices abstractly, while <see cref="PointD"/> describes a concrete point in 2D space. This
///         class is intended to allow vertices to be reliably compared for equality without the pitfall of floating-point
///         rounding errors throwing off equality comparisons.</para></remarks>
public abstract class Vertex : IEquatable<Vertex>
{
    ...
    ///     <paramref name="from"/> to this one.</summary>
    /// <param name="from">
    ...
    ///     cref="Point"/> directly to allow client code to use <see cref="SvgInstructions.GetVertexPoint"/> to override the
    ///     positions of vertices.</param>
    /// <param name="isLast">
    ///     <c>true</c> if we are generating a closed path and this is the last invocation. This vertex is therefore the first
    ///     in the path, as the path is returning to its starting position.</param>
    /// <param name="r">
    ...
    /// <remarks>
    ///     The implementation must assume that the SVG path is already at the <paramref name="from"/> vertex and return only
    ///     the part of the path that leads to the current vertex. For example, the default implementation returns
    ///     <c>$"L{<paramref name="r"/>(<see cref="PointD.X"/>)} {<paramref name="r"/>(<see cref="PointD.Y"/>)}"</c> and does
    ///     not reference <paramref name="from"/> at all.</remarks>
