...
public class Structure<TCell>
{
    ...
    /// <param name="links">
    ///     An explicit set of links (connections) between cells, defining which pairs of cells are neighbors. See remarks.</param>
    ...
    ///     <list type="bullet">
    ///         <item><description>
    ///             <paramref name="links"/> is specified and <paramref name="getNeighbors"/> is <c>null</c>. In this case,
    ///             the explicit set of links is used.</description></item>
    ///         <item><description>
    ///             <paramref name="getNeighbors"/> is specified and <paramref name="links"/> is <c>null</c>. In this case,
    ///             the delegate is called on every cell to discover all links. The delegate is allowed to return additional
    ///             cells that are not in <paramref name="cells"/>; those will be discarded.</description></item>
    ///         <item><description>
    ///             <typeparamref name="TCell"/> implements <see cref="INeighbor{TCell}"/>, in which case the behavior is the
    ///             same as with <paramref name="getNeighbors"/> but <see cref="INeighbor{TCell}.Neighbors"/> is used to
    ///             retrieve the neighbors.</description></item></list></remarks>
    ...
    ///     name="link"/>. This method is only called for links between cells that have no edges in common; otherwise, <see
    ///     cref="svgEdgeType(Link{Vertex}, List{TCell})"/> is called instead.</summary>
    /// <param name="link">
    ...
    private static string svgEdgePath(Link<Vertex> segment, Func<Vertex, PointD> getVertexPoint, Func<double, string> r)
    {
    ...
    /// <summary>
    ///     Finds the shortest path from the specified <paramref name="origin"/> cell to every other cell in the structure.</summary>
    /// <param name="origin">
    ///     The start cell at which all paths begin.</param>
    /// <returns>
    ///     A dictionary mapping from each cell to a structure revealing the distance from <paramref name="origin"/> as well
    ///     as the previous cell on the path.</returns>
    ...
    /// <returns>
    ///     A collection containing every cell along the path, including <paramref name="from"/> and <paramref name="to"/>.</returns>
    ...
    ///         <c>Structure&lt;CombinedCell&lt;TCell&gt;&gt;</c>. If you wish to combine multiple groups of cells, call this
    ///         overload with zero parameters first just to change the type, then subsequently use <see
